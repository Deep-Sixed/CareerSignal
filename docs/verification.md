...
| Tests | 23 tests: normalization, distinct jobs within one message, message/opportunity dedupe, strict score threshold, eligibility, stale approvals, transaction rollback/commit, foreign keys, migration repeat/failure/upgrade/checksum, two-connection retry/exhaustion, concurrent migrations, contended and multi-process first-start initialization, immutable audit, concurrent draft claims, uncertain external results and receipt reconciliation. |
