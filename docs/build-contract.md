...
**Web surface decision — 2026-09-14.** The local read surface is served by Python's standard-library `http.server`, with no web framework, no ASGI/WSGI server, no JavaScript toolchain, and no `careersignal[web]` extra. The wheel stays `py3-none-any` with zero `Requires-Dist`, and the twelve-cell Linux/macOS/Windows × Python 3.11–3.14 matrix is unaffected, because the surface is one loopback socket serving read projections that are already decided and packaged static assets that are already in the wheel. FastAPI, Flask, Starlette, Uvicorn and a Node/npm build step were all considered and refused: each adds a transitive dependency tree, a platform surface, and a release cadence to an application whose entire threat model is "nothing leaves this machine", and none of them answers a question `http.server` cannot. Server-side templating is refused for the same reason: `json.dumps` remains the escaping boundary, and recruiter-controlled evidence is never interpolated into HTML.

The surface is confined by construction rather than by configuration. It binds `127.0.0.1` and offers no option to bind anything else. It answers `GET` and `HEAD` and refuses every other method before routing, so no request shape reaches a write. It reaches the repository's read projections and nothing else -- no intake, no decision, no claim, no provider, no credential -- and a mechanical guard over `src/system/web` fails the build if that changes. Authority is a fresh high-entropy token minted per launch and held in process memory only: never written to the database, to configuration, to an environment variable, to a file, or to a log, and never reported by the API, which may state only whether a credential is present. It travels to the browser in the URL fragment, which is not sent in an HTTP request, and returns in the `X-CareerSignal-Token` header, compared with `secrets.compare_digest`. An unexpected `Host` or `Origin` is refused and no `Access-Control-Allow-*` header is ever emitted, which closes DNS rebinding. Static assets are packaged resources discovered with `importlib.resources` and served by exact name, so there is no path to traverse. Revisit this decision only when a concrete CareerSignal requirement cannot be met by the standard library, and only after the candidate passes the same installed-wheel, zero-dependency and supported-platform gates as the storage engine.

**Presentation-derivation deviation — 2026-09-14.** The frozen design handoff directs the interface to port `views.coverage()`, `views.queue()`, `views.approval()` and `views.attempt()` verbatim to JavaScript. That instruction is not followed, and the accepted visual design is unchanged by the decision. Those four functions stay in `system/views.py`, which already declares itself presentation-only, and the HTTP surface attaches their output to a response when `presentation=true` is asked for. The handoff was written before the loopback read surface existed; at that point a browser deriving its own labels duplicated nothing, because there was no other place the derivation could live. There is now. `queue()`'s precedence is a rule rather than a formatter -- a durable intent outranks anything the audit trail implies -- and it is mutation-tested where it lives. A second implementation in a language with no test runner in this repository could drift from it silently, and the symptom would be a list and a detail pane disagreeing about the same opportunity, which is the exact failure `queue()` exists to prevent. Adding a JavaScript runtime to prove the copy behaves would mean putting Node into twelve CI cells to test a duplicate of something already proven, which trades the zero-dependency contract for a worse position than not duplicating at all. The mock remains authoritative about what the operator sees; Python remains authoritative about what that state means. `docs/ui-design/` is not edited: it records the plan as accepted, and this records where the implementation departed from it and why.
