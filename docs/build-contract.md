...
│   │   └── tests/              # real libSQL and concurrency tests
│   └── system/
...
| `data` | libSQL connection/transactions, repositories, relational schemas, migrations, durable audit/idempotency state | Mailbox calls, HTTP routes, UI logic |
...
`recruiting` is the domain core and owns the interfaces it needs. Keep it independent of Gmail, libSQL, and the application shell.
...
- Python `>=3.11,<3.15`; exact `libsql==0.1.11` dependency, resolved in `uv.lock`.
...
- Full dependency installation and `pip check` are required. A `--no-deps` storage test alone cannot establish application portability.
...
The local baseline can use deterministic scoring and a controlled in-memory communications adapter. It must still exercise real libSQL persistence, explicit approval, audit records, and replay behavior end to end. Do not use a mock to claim live Gmail integration is verified.
...
2. **Scope/tree:** every source file has an owner; dependency direction is checked; no legacy infrastructure, duplicate implementations, or case collisions.
...
4. **Storage:** real libSQL contract, migrations, constraints, transaction/concurrency and retry tests pass.
5. **Installed artifact:** wheel builds, installs with dependencies into an empty environment, passes dependency consistency, imports from that installation, includes migrations, excludes private/test/runtime assets, and runs outside the checkout.
...
7. **Quality/CI readiness:** relevant tests and Ruff pass locally; workflow definitions cover Linux/macOS/Windows and Python 3.11–3.14 with the documented Windows/3.14 libSQL exception. Clearly distinguish locally tested environments from pending hosted CI.
